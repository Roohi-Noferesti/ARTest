...
/// </summary>
public class ARInteractSpawnTrigger : MonoBehaviour
{
    /// <summary>
    ...
    [SerializeField]
    [Tooltip("Reference to the menu manager which will be opened on tap instead of spawning immediately.")]
    ARMenuManager m_MenuManager;

    public ARMenuManager menuManager
    {
        get => m_MenuManager;
...
#if UNITY_2023_1_OR_NEWER
            m_MenuManager = FindAnyObjectByType<ARMenuManager>();
#else
        m_MenuManager = FindObjectOfType<ARMenuManager>();
#endif
    }
    ...
    void Update()
    {
        ...
        if (m_AttemptSpawn)
        {
            ...
            if (!isPointerOverUI && m_ARInteractor.TryGetCurrentARRaycastHit(out var arRaycastHit))
            {
                ...
                if (m_MenuManager != null)
                {
                    m_MenuManager.OpenMenuAt(arRaycastHit.pose.position, arPlane.normal);
                }
                else
                {
                    // fallback: spawn immediately if no menu manager assigned
